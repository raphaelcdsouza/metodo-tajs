...
When('I create a new senior user with the following details:', async function(dataTable) {
...
Then('I request the API with the senior user\'s ID', async function() {
  const user = await findUserById(_context.userData.id)
  _context.createdUserData = user
})

Then('I should receive a JSON response with the senior user\'s details', function() {
  const expectedKeys = [
    'name',
    'birthDay',
    'id',
...
Then('the senior user\'s category should be {string}', function(category) {
